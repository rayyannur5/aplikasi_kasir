...
class LaporanAIDevicePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Ringkasan AI Device')),
          ...
          children: [
            ref.watch(futureGetOutletsProvider("")).when(
                  data: (data) => DropdownButtonFormField(
                      items: data.map((e) => DropdownMenuItem(child: Text(e['nama']), value: e)).toList(),
                      value: data.first,
                      onChanged: (value) {
                  ...
                  loading: () => Shimmer.fromColors(child: Container(height: 55, color: Colors.black), baseColor: Colors.transparent, highlightColor: Colors.white.withOpacity(0.5)),
                ),
            ...
            ref.watch(futureReportAIDeviceProvider(parameterAI)).when(
                  ...
                  loading: () => Column(
                    children: [
                      Shimmer.fromColors(child: Card(child: SizedBox(height: 200, width: size.width - 40)), baseColor: Colors.transparent, highlightColor: Colors.white.withOpacity(0.5)),
                      Shimmer.fromColors(child: Card(child: SizedBox(height: 300, width: size.width - 40)), baseColor: Colors.transparent, highlightColor: Colors.white.withOpacity(0.5)),
                    ],
                  ),
                )
          ],
